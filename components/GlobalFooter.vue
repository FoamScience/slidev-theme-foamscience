<script setup lang="ts">
import { computed } from 'vue'
import { useNav, useSlideContext } from '@slidev/client'

const nav = useNav()
const { currentSlideNo, total, currentLayout } = nav
const { $slidev } = useSlideContext()

// Footer configuration from global config (which includes frontmatter from first slide)
const footerConfig = computed(() => $slidev?.configs?.footer || {})

const author = computed(() => footerConfig.value.author || '')
const affiliation = computed(() => footerConfig.value.affiliation || '')
const github = computed(() => footerConfig.value.github || '')
const linkedin = computed(() => footerConfig.value.linkedin || '')

// Show footer on all slides except cover and end
const showFooter = computed(() => {
  return currentLayout.value !== 'cover' && currentLayout.value !== 'end'
})
</script>

<template>
  <footer v-if="showFooter" class="global-footer">
    <div class="footer-content">
      <!-- Left side: Author info -->
      <div v-if="author || affiliation" class="footer-left">
        <span v-if="author" class="footer-author">{{ author }}</span>
        <span v-if="author && affiliation" class="footer-separator">•</span>
        <span v-if="affiliation" class="footer-affiliation">{{ affiliation }}</span>
      </div>

      <!-- Right side: Social links and slide counter -->
      <div class="footer-right">
        <!-- GitHub link -->
        <a
          v-if="github"
          :href="github.startsWith('http') ? github : `https://github.com/${github}`"
          target="_blank"
          rel="noopener noreferrer"
          class="footer-link"
          title="GitHub">
          <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="currentColor">
            <path d="M12 0c-6.626 0-12 5.373-12 12 0 5.302 3.438 9.8 8.207 11.387.599.111.793-.261.793-.577v-2.234c-3.338.726-4.033-1.416-4.033-1.416-.546-1.387-1.333-1.756-1.333-1.756-1.089-.745.083-.729.083-.729 1.205.084 1.839 1.237 1.839 1.237 1.07 1.834 2.807 1.304 3.492.997.107-.775.418-1.305.762-1.604-2.665-.305-5.467-1.334-5.467-5.931 0-1.311.469-2.381 1.236-3.221-.124-.303-.535-1.524.117-3.176 0 0 1.008-.322 3.301 1.23.957-.266 1.983-.399 3.003-.404 1.02.005 2.047.138 3.006.404 2.291-1.552 3.297-1.23 3.297-1.23.653 1.653.242 2.874.118 3.176.77.84 1.235 1.911 1.235 3.221 0 4.609-2.807 5.624-5.479 5.921.43.372.823 1.102.823 2.222v3.293c0 .319.192.694.801.576 4.765-1.589 8.199-6.086 8.199-11.386 0-6.627-5.373-12-12-12z"/>
          </svg>
        </a>

        <!-- LinkedIn link -->
        <a
          v-if="linkedin"
          :href="linkedin.startsWith('http') ? linkedin : `https://linkedin.com/in/${linkedin}`"
          target="_blank"
          rel="noopener noreferrer"
          class="footer-link"
          title="LinkedIn">
          <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="currentColor">
            <path d="M19 0h-14c-2.761 0-5 2.239-5 5v14c0 2.761 2.239 5 5 5h14c2.762 0 5-2.239 5-5v-14c0-2.761-2.238-5-5-5zm-11 19h-3v-11h3v11zm-1.5-12.268c-.966 0-1.75-.79-1.75-1.764s.784-1.764 1.75-1.764 1.75.79 1.75 1.764-.783 1.764-1.75 1.764zm13.5 12.268h-3v-5.604c0-3.368-4-3.113-4 0v5.604h-3v-11h3v1.765c1.396-2.586 7-2.777 7 2.476v6.759z"/>
          </svg>
        </a>

        <!-- Slide counter -->
        <div class="footer-slide-counter">
          {{ currentSlideNo }} / {{ total }}
        </div>
      </div>
    </div>
  </footer>
</template>

<style scoped>
.global-footer {
  position: fixed;
  bottom: 0;
  left: 0;
  right: 0;
  z-index: 10;
  pointer-events: none;
  background-color: var(--c-mantle);
}

.footer-content {
  display: flex;
  justify-content: space-between;
  align-items: center;
  padding: 0.5rem 1rem;
  font-size: 0.875rem;
  color: var(--c-subtext0);
}

.footer-left {
  display: flex;
  align-items: center;
  gap: 0.375rem;
  pointer-events: auto;
}

.footer-author {
  font-weight: 500;
  color: var(--c-text);
}

.footer-separator {
  color: var(--c-surface2);
}

.footer-affiliation {
  color: var(--c-subtext1);
  font-style: italic;
}

.footer-right {
  display: flex;
  align-items: center;
  gap: 0.5rem;
  pointer-events: auto;
}

.footer-link {
  display: flex;
  align-items: center;
  justify-content: center;
  width: 2rem;
  height: 2rem;
  color: var(--c-subtext0);
  transition: all 0.2s ease;
  border-radius: 0.375rem;
  text-decoration: none;
}

.footer-link:hover {
  color: var(--c-blue);
  background-color: var(--c-surface0);
}

.footer-link svg {
  width: 1rem;
  height: 1rem;
}

.footer-slide-counter {
  font-size: 0.875rem;
  color: var(--c-subtext0);
  font-variant-numeric: tabular-nums;
  min-width: 3.5rem;
  text-align: right;
  font-weight: 500;
}
</style>
